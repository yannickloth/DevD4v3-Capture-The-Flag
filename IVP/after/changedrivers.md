...
| Empty marker interfaces | an empty interface (e.g. `ISystem`) declares no contract and carries **no change driver**. Framework registration / ECS membership is captured by the concrete member's drivers, not by the marker |
