...
/// <remarks>Change drivers: CD-29 (code-under-test: the in-memory repository seam); CD-20 (outbound repository contract); CD-18 (database schema/player data model: the <c>Dictionary&lt;int,FakePlayer&gt;</c> store); CD-21 (DI container/composition); CD-25 (BCrypt password-hashing contract via the fake hasher). Carries no SQL-dialect driver (dialect-free store).</remarks>
public class InMemoryRepositoryManager : IRepositoryManager
{
    ...
    /// <remarks>Change drivers: CD-20 (outbound repository contract), CD-18 (database schema/player data model), CD-21 (DI container/composition), CD-25 (BCrypt password-hashing contract).</remarks>
